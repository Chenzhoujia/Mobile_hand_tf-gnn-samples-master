...
LOG_DIR = './tf-dev-summit-tensorboard-tutorial-master'
...
def create_sprite_image(images):
    # permutation = np.random.permutation(images.shape[0])  # 记下第一维度的打乱顺序
    # images = images[permutation, :, :]  # 按照顺序索引
...
sprite_image = create_sprite_image(to_visualise[:1024])
...
with open(path_for_mnist_metadata, 'w') as f:
    for index, label in enumerate(mnist.test.labels[:1024]):
        #f.write("%d\t%d\n"%(index, label))
        f.write("%d\n"%(1))